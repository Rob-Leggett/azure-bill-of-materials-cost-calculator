from decimal import Decimal
from typing import Dict

from ..helpers.math import decimal
from ..helpers.pricing import price_by_service
from ..types import Key

def price_synapse(component, region, currency, ent_prices: Dict[Key, Decimal]):
    # If your CSV names this "Azure Synapse Analytics", pass that in component["service"].
    service = (component.get("service") or "Azure Synapse Analytics").strip()
    sku     = (component.get("sku") or "").strip()                  # e.g., DW100c or specific pool SKU
    uom     = (component.get("uom") or "1 Hour").strip() or None    # per DWU/CDWU hour if applicable
    qty     = decimal(component.get("quantity", component.get("dwu", component.get("instances", 1))))
    hours   = decimal(component.get("hours_per_month", 730))

    return price_by_service(
        service=service, sku=sku, region=region, currency=currency, ent_prices=ent_prices,
        uom=uom, qty=qty, hours=hours, must_contain=[sku.lower()] if sku else None
    )